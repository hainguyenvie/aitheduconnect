...
interface CourseCardProps {
  course: any; // Supabase course with teachers joined
  progress?: number;
}
  ...
  return (
    <div className="bg-white rounded-xl shadow-md overflow-hidden flex flex-col h-full relative">
      ...
      <div className="flex-1 flex flex-col p-4">
        ...
        {/* Short description */}
        <p className="text-gray-600 text-sm mb-2 line-clamp-2">{description || 'Khóa học chất lượng cao, phù hợp cho mọi đối tượng.'}</p>
        {/* Teacher info */}
        <div className="flex items-center gap-2 mb-2">
          {teachers?.avatar ? (
            <img src={teachers.avatar} alt={teachers.full_name} className="w-7 h-7 rounded-full object-cover border" />
          ) : (
            <div className="w-7 h-7 rounded-full bg-gray-300 flex items-center justify-center text-xs font-bold text-white">
              {getInitials(teachers?.full_name || "?")}
            </div>
          )}
          <span className="text-sm font-medium">{teachers?.full_name}</span>
        </div>
        {/* Stats */}
        <div className="flex items-center text-xs text-gray-500 gap-4 mb-2">
          <span><span className="font-semibold">{total_sessions}</span> buổi</span>
